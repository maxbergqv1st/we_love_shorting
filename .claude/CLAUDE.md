...
- **Forecast horizon:** `controller.shift_target(df, target, horizon)` turns the nowcast frame into a forecast frame (row t keeps its features, target becomes t+horizon's actual; last rows dropped) — shift happens **before** any split so nothing leaks, and every downstream piece (baseline, direction, live panel) works unchanged. The app's Horisont picker: Idag (0) / Imorgon (1).
