...
- **Charts:** all multi-series lines go through app's `line_chart()` — Altair **without pan/zoom** (native `st.line_chart` hijacks the mouse wheel) with tooltips. Categorical series → scatter/heatmap, never a line. Correlation heatmap = `Styler.map(_corr_cell)` (manual red/green rgba — **not** `background_gradient`, which needs matplotlib). **No in-sample error charts** — in-sample error reads as skill (a forest fits ~perfectly); held-out views only (the residual histogram). Cut deliberately: the in-sample error line, the per-feature small-chart grid, and the compare card's group-mean indexed line (a mean of indexed price levels is uninterpretable and duplicated the grouping card) — don't reintroduce.
...
- **Level targets train on their MOVE (`controller.prepare_target`):** a price level is a near-random-walk — trained on the level, the model mostly re-learns persistence. So `X_close` trains on the existing `X_ret`, `tone` on a computed `tone_diff`, a `_ret` pick is identity; the re-target happens **BEFORE** the horizon shift (shifting the level and re-targeting after would leave the move unshifted). `TargetSpec(train_col, level_col, kind, horizon)` flows to the modes — they train on `spec.train_col`; Direction consumes the same move column (no suffix-sniffing). **Charts stay on the move scale — never draw a reconstructed level curve**: rebuilt on the previous ACTUAL level it hugs the actual by construction (persistence, not skill) and hides the model's up/down call. Reconstruction (`level_base` — horizon≥1: the row's own level col; nowcast: backed out per-row `level ⊖ actual move` — + `reconstruct_level`: × for ret, + for diff) is used only for the live card's forecast figure and the level-scale metrics. Eval shows both scales: move vs mean-baseline (the honest verdict) and reconstructed level vs persistence — persistence ≡ predict-zero-move, so beating it on the level ⟺ beating always-0 on the move.
- **Forecast horizon:** `controller.shift_target(df, target, horizon)` (called via `prepare_target`) turns the nowcast frame into a forecast frame (row t keeps its features, target becomes t+horizon's actual; last rows dropped) — shift happens **before** any split so nothing leaks, and every downstream piece (baseline, direction, live panel) works unchanged. The app's Horisont picker: Idag (0) / Imorgon (1). **The shift runs over TRADING days** (closed rows dropped first): the raw frame has one row per tone date, so a calendar shift hands Friday rows Saturday's carried-forward close — their own value — poisoning ~31% of rows with fake "tomorrow = today" examples. The live panel takes both frames: the shifted one to train, the raw one for live quote/fallback values (the shifted tail is stale).
